...
namespace CommandLineEncoder
{
   ...
   class Program
   {
      ...
      static void Main()
      {
          ...
          var sr = new StreamReader(AppDomain.CurrentDomain.BaseDirectory + "QRList.tsv", System.Text.Encoding.ASCII);

          while (sr.Peek() > -1)
          {
              var line = sr.ReadLine();
              args[0] = line.Substring(6);
              args[1] = String.Format("--output={0}",line.Substring(0, 5) + ".png");
              args[2] = "--image_format=png";
              execute(args);
          }
      ...
      private static void execute(string[] args)
      {
         ...
         if (contents == null)
         {
          ...
          var bitmatrix = qrcode.encode(contents, BarcodeFormat.QR_CODE, 100, 100, hints);
          Bitmap bitmap = new Bitmap(100, 100, System.Drawing.Imaging.PixelFormat.Format24bppRgb);
